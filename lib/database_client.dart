...
//insertion
  Future<int> saveItem(noDueTask item) async {
...
//
  Future<noDueTask> getItem(int id) async {
    var dbClient = await db;
    var result = await dbClient.rawQuery("SELECT * FROM $tableName WHERE id = $id");
    if (result.length == 0) return null;
    return new noDueTask.fromMap(result.first);
  }
...
//  }
  ...
  Future<int> updateItem(noDueTask item) async {
    var dbClient = await db;
